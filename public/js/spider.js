~(function($){
	$.fn.spider = function(option){
		...
		<iframe id="iframedata" name="iframedata" style="display: none" ></iframe>
		...
		socket.on(director,function(msg){
			...
			if($("#datadetail").text() === "爬取完毕")
			{
				...
				(function(callback){
					...
					var ospan = $("#iframedata").contents().find("body").children();
					if(ospan.length>0)
					{
						clearInterval(timer);
						var content = $("#iframedata").contents().find("#loadfile").text();
						var loadfileurl = content.split("/links/")[1];
